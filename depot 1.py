...
Q = 1500  # vehicle capacity
no_of_vehicles = 2  # free to modify
no_of_customers = 3
df = pd.read_csv(r'C:\Users\Dell\OneDrive\Desktop\pr2\depot 1.txt', sep=" ")
...
'''objective function'''
m.setObjective(quicksum(quicksum(x[(i, j)] * dist_matrix[(i, j)] for j in range(n))*80+1000 for i in range(n)), GRB.MINIMIZE)
m.update()
...
def plot_tours(solution_x):
    ...
    plt.scatter(df["X"][0], df["Y"][0], marker = "o", color = 'b', label = "Depot 1")
