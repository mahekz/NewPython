...
no_of_vehicles = 10  # free to modify
no_of_customers = 16
df = pd.read_csv(r'C:\Users\Dell\OneDrive\Desktop\pr2\satellite 7.txt', sep=" ")
...
'''objective function'''
m.setObjective(quicksum(quicksum(x[(i, j)] * dist_matrix[(i, j)] for j in range(n))*50+1000 for i in range(n)), GRB.MINIMIZE)
m.update()
...
def plot_tours(solution_x):
    ...
    plt.scatter(df["X"][0], df["Y"][0], marker = "o", color = 'b', label = "Satellite 7")
